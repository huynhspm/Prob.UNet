...
def get_msmri_dataset(args, mode: str = ""):
    if mode == "train":
        train_dataset = MSMRIDataset(data_dir=args.data_dir, train_val_test_dir="Train", mask_type=args.mask_type)
        train_dataset = TransformDataset(train_dataset, image_size=args.image_size)
    
    if mode == "val":
        val_dataset = MSMRIDataset(data_dir=args.data_dir, train_val_test_dir="Val", mask_type=args.mask_type)
...
class MSMRIDataset(Dataset):
    ...
    def __init__(
        ...
        if train_val_test_dir:
            img_dirs = [f"{data_dir}/{train_val_test_dir}/*/time_point_*_image_slice_*.npy"]
        else:
            img_dirs = [
                f"{data_dir}/Train/*/time_point_*_image_slice_*.npy",
                f"{data_dir}/Val/*/time_point_*_image_slice_*.npy",
            ]
...
if __name__ == "__main__":
    dataset = MSMRIDataset(data_dir='data/msmri', mask_type="multi")
    print(len(dataset))
    ...
    axs[1, 0].imshow(image[:,:,3], cmap='gray')
    ...
    sns.heatmap(mask_var, ax=axs[1, 2], cmap="viridis")
    axs[1, 2].set_title('Variance Heatmap')
    axs[1, 2].axis("off")
    
    axs[2, 0].imshow(masks[:, :, 0], cmap='gray')
    axs[2, 0].set_title('Mask_1')
    axs[2, 0].axis("off")
    
    axs[2, 1].imshow(masks[:,:,1], cmap='gray')
    axs[2, 1].set_title('Mask_2')
    axs[2, 1].axis("off")
    
    axs[2, 2].imshow(mask_e, cmap='gray')
    axs[2, 2].set_title('Mask_e')
    axs[2, 2].axis("off")
